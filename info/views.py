...
def custom_login(request):
    """
    Custom login view that uses name-based authentication
    """
    if request.user.is_authenticated:
        return redirect('index')
    
    if request.method == 'POST':
        form = NameLoginForm(request, data=request.POST)
        if form.is_valid():
...
def change_password(request):
    """
    Password change view for students
    """
    if not request.user.is_student:
        messages.error(request, 'Password change is only available for students.')
        return redirect('index')
    ...
    return render(request, 'info/change_password.html', {'form': form})
